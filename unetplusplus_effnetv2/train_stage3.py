...
def unetplusplus_train(epochs, gpu, base_model_name, encoder_weights, generator_lr, discriminator_lr, lambda_loss,
                batch_size, image_train_dir, mask_train_dir, image_test_dir, mask_test_dir):
    # make directory
    weight_path = './UnetPlusPlus/stage3_dibco_' + base_model_name + '_' + str(int(lambda_loss)) + '_' + str(generator_lr)
    ...
    for epoch in range(epochs):
        ...
        for idx, (images, masks) in enumerate(patch_train_loader):
            ...
            if idx % 2000 == 0:
                print('train step[%d/%d] patch discriminator loss: %.5f, total loss: %.5f, generator loss: %.5f, bce loss: %.5f, dice loss: %.5f, time: %.2f' % 
                            (idx, len(patch_train_loader), discriminator_loss.item(), total_loss.item(), generator_loss.item(), bce_loss.item(), dice_loss.item(), time.time() - epoch_start_time))
...
if __name__ == "__main__":
    ...
    # data set
    parser.add_argument('--image_train_dir', type=str, default='./UnetPlusPlus/predicted_image_for_stage3_dibco_UnetPlusPlus/train/patch/image/', help='patched stage2 prdiction image train dir')
    parser.add_argument('--mask_train_dir', type=str, default='./UnetPlusPlus/predicted_image_for_stage3_dibco_UnetPlusPlus/train/patch/mask/', help='patched stage2 prdiction mask train dir')
    parser.add_argument('--image_test_dir', type=str, default='./UnetPlusPlus/predicted_image_for_stage3_dibco_UnetPlusPlus/test/', help='stage2 prdiction image test dir')
    parser.add_argument('--mask_test_dir', type=str, default='../Testset/mask/', help='original mask test dir')
    
    opt = parser.parse_args()
    unetplusplus_train(opt.epochs, opt.gpu, opt.base_model_name, opt.encoder_weights, opt.generator_lr, opt.discriminator_lr, opt.lambda_loss,
