...
class Dataset_Return_Four(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask = np.uint8(cv2.resize(mask,(128,128),interpolation=cv2.INTER_AREA))
        ...
        image_gray_LL,(image_gray_LH,image_gray_HL,image_gray_HH) = dwt2(image_gray,'db1')
        ...
        nr0, nc0 = image_gray_LL.shape[:2]
        image_gray_LL_norm = np.zeros( [nr0, nc0] )
        cv2.normalize( image_gray_LL, image_gray_LL_norm, 0, 255, cv2.NORM_MINMAX )
        ...
        image_gray_LL_norm = np.uint8(image_gray_LL_norm)
        image_blue_LL_norm = np.uint8(image_blue_LL_norm)
        image_green_LL_norm = np.uint8(image_green_LL_norm)
        image_red_LL_norm = np.uint8(image_red_LL_norm)

        image_gray_LL_norm = np.expand_dims(image_gray_LL_norm, axis=-1)
        ...
        image_gray_LL_norm = self.preprocess_input(image_gray_LL_norm)
        ...
        image_gray_LL_norm = torch.from_numpy(image_gray_LL_norm).permute(2, 0, 1).float()
        ...
        return (image_blue_LL_norm, image_green_LL_norm, image_red_LL_norm, image_gray_LL_norm), (mask_and_blue, mask_and_green, mask_and_red, mask)
