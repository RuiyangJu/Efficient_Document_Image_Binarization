...
import numpy as np
import cv2
from Base.tool_clean import check_is_image, image_padding
import csv

def main(argv=None):

    image_dir = ['./Trainset/image/', './Testset/image/']
    mask_dir = ['./Trainset/mask/', './Testset/mask/']
    ...
    image_save_dir = ['./Ours_resize_512/Trainset/image/', './Ours_resize_512/Testset/image/']
    mask_save_dir = ['./Ours_resize_512/Trainset/mask/', './Ours_resize_512/Testset/mask/']
    ...
    for image_dir, mask_dir, image_save_dir, mask_save_dir in zip(image_dir, mask_dir, image_save_dir, mask_save_dir):
        ...
        for image in image_list:
            ...
            elif os.path.isfile(os.path.join(mask_dir, img_name) + '.jpg'):
                mask = cv2.imread(os.path.join(mask_dir, img_name) + '.jpg', cv2.IMREAD_GRAYSCALE)
            else:
                print('no mask')
                continue
