...
weight_folder = './Unet/stage2_dibco_' + base_model_name + '_' + str(int(lambda_loss)) + '_' + str(generator_lr) + '_' + str(threshold)  + '/'
...
# make directory
image_save_path = './Unet/predicted_image_for_stage3_dibco_Unet'
...
for img in tqdm(images):
    ...
    elif os.path.isfile(os.path.join(mask_train_dir, image_name + '.jpg')):
        mask = cv2.imread(os.path.join(mask_train_dir, image_name + '.jpg'), cv2.IMREAD_GRAYSCALE)
    else:
        print(img, 'no mask')
        exit(1)
