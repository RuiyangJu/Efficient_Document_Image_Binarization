...
def unet_train(epochs, gpu, base_model_name, encoder_weights, generator_lr, discriminator_lr, lambda_loss, threshold,
                batch_size, image_train_dir, mask_train_dir, image_test_dir, mask_test_dir):
    # make save directory
    weight_path = './Unet/stage2_dibco_' + base_model_name + '_' + str(int(lambda_loss)) + '_' + str(generator_lr) + '_' + str(threshold) 
...
if __name__ == "__main__":
    ...
    unet_train(opt.epochs, opt.gpu, opt.base_model_name, opt.encoder_weights, opt.generator_lr, opt.discriminator_lr, opt.lambda_loss, opt.threshold,
